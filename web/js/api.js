...
const API = (function () {
  ...
  function mockHandler(method, path, body) {
    ...
    // Addresses
    if (path === '/user/addresses') return ADDRESSES;
    if (path === '/user/addresses' && method === 'POST') {
      const a = { id: Date.now(), isDefault: false, ...body };
      ADDRESSES.push(a); return a;
    }
    if (path.startsWith('/user/addresses/') && method === 'PUT') {
      const id = parseInt(path.split('/')[2]);
      const idx = ADDRESSES.findIndex(a => a.id === id);
      if (idx >= 0) ADDRESSES[idx] = { ...ADDRESSES[idx], ...body };
      return ADDRESSES[idx];
    }
    if (path.startsWith('/user/addresses/') && method === 'DELETE') {
      const id = parseInt(path.split('/')[2]);
      const idx = ADDRESSES.findIndex(a => a.id === id);
    ...
    // Coupons
    if (path === '/user/coupons') return COUPONS;
    if (path === '/user/coupons') return COUPONS.filter(c => c.status === 0);
  ...
  return {
    ...
    getAvailableCoupons: async () => { const d = await get('/user/coupons'); const l = (d && d.list) || d || []; return Array.isArray(l) ? l.filter(c => c.status === 0) : l; },
